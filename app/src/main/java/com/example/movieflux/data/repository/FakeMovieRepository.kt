...
// Singleton so all ViewModels share the same _favoriteIds state (cross-screen sync).
...
object FakeMovieRepository : MovieRepository {
    ...
    override fun getMovieDetail(id: Int): Flow<MovieModel> = _favoriteIds.map { ids ->
        allMovies.find { it.id == id }?.copy(isFavorite = id in ids)
            ?: throw NoSuchElementException("Movie not found: $id")
    }
